...
int main(int argc, char* argv[]) {
  ...
  /*ze_image_format_t format = {
    ...
    ZE_IMAGE_FORMAT_SWIZZLE_R, ZE_IMAGE_FORMAT_SWIZZLE_0, ZE_IMAGE_FORMAT_SWIZZLE_1
  };
  ...
  ze_image_handle_t srcImage, destImage;
  errno = zeImageCreate(hContext, hDevice, &imageDesc, &srcImage);
  check_error(errno, "zeImageCreate");
  errno = zeImageCreate(hContext, hDevice, &imageDesc, &destImage);
  check_error(errno, "zeImageCreate");
  */
