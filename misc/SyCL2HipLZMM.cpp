...
int hiplzInit(void* driverPtr, void* deviePtr, void* contextPtr, void* queueptr) {
  hipInitFromOutside(driverPtr, deviePtr, contextPtr, queuePtr);
}

int RunMMTest(void* driverPtr, void* deviePtr, void* contextPtr, void* queuePtr) {
  
  hipInitFromOutside(driverPtr, deviePtr, contextPtr, queuePtr);
  std::cout << "After initialization from outside" << std::endl;
  ...
  std::cout << "Before get device prop" << std::endl;
  hipDeviceProp_t devProp;
  err = hipGetDeviceProperties(&devProp, 0);
  std::cout << "After get device prop"	<< std::endl;
  ERR_CHECK;
