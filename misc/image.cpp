...
int main() {
  ...
  try {
    ...
    if (Devs.size() == 1) {
      std::cout << "SINGLE DEVICE CASE" << std::endl;
      RunMMTest(Plt.template get_native<sycl::backend::level_zero>(),
      ...
      std::cout << "MULTIPLE DEVICE CASE" << std::endl;
    // auto NativePlt = Plt.get_native<backend::level_zero>();
    // ctx.get_native<sycl::backend::level_zero>();
    // std::cout << "native handler: " << get_native<backend::level_zero, queue>(myQueue) << std::endl;
    int backend_name = (int)myQueue.get_backend();

    std::cout << "backend name:  " << backend_name << " and: " << (int)backend::level_zero << std::endl;
    std::cout << "native handle: " << myQueue.template get_native<sycl::backend::level_zero>() << std::endl;
