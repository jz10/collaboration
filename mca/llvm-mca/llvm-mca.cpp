...
public:
  ...
  void HandleComment(SMLoc Loc, StringRef CommentText) override {
    ...
    if (CommentStr.length() >= 16) {
      std::string subStr = CommentStr.substr(0, 16); 
      EndFunction = (subStr == " -- End function");
      Regions.endRegion(Loc);
      return;
    }
    if (CommentStr.length() >= 19) {
      std::string subStr = CommentStr.substr(0, 18);
      BeginFunction = (subStr == " -- Begin function");
      FunctionName = CommentStr.substr(19, CommentStr.length() - 19);
      Comment.drop_front(19);
...
public:
  ...
  bool EmitSymbolAttribute(MCSymbol *Symbol, MCSymbolAttr Attribute) override {
    ...
    if (Attribute == MCSA_ELF_TypeFunction) {
      ...
      std::cout << "Function Name: " << Name.str() << std::endl;
      Regions.beginRegion(Name, SMLoc()); 
    }
    return true;
  }
...
int main(int argc, char **argv) {
  ...
  AsmStreamer.reset(&Str);
  LLVMTargetMachine* LLVMTM = 0;
  FunctionPass *Printer_ = LLVMTM->getTarget().createAsmPrinter(* LLVMTM, std::move(AsmStreamer));
  */
  ...
  for (const std::unique_ptr<mca::CodeRegion> &Region : Regions) {
    ...
    if (Region->empty()) {
      std::cout << "Empty Region?";
      continue;
    } else
      std::cout << "Proc Region: " << Region->getDescription().str() << std::endl;
